...
lines.append(f"""  Storage         : {ssd_model}  ({ssd_capacity})
  ...
  Container Free  : {disk_free_gb:.1f} GB  (System Settings may show ~5-10 GB more — purgeable caches)
...
# ── 14. POWER MANAGEMENT ──────────────────────────────────────────────────────
lines.append(section("💤  POWER MANAGEMENT"))
lines.append(f"""  Power Source    : {pmset_source}
...
_psutil_status = "✅ enabled" if HAS_PSUTIL else "not installed (run via run.sh for auto-install)"
lines.append(f"{'─'*66}")
lines.append(f"  Generated by battery_info.py · Python {py_ver} · psutil {_psutil_status}")
lines.append("")
